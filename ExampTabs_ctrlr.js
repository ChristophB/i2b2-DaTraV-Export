...
i2b2.ExampTabs.getResults = function() {
    ...
    $$("DIV#ExampTabs-mainDiv DIV#ExampTabs-TABS DIV.results-invalidQuery")[0].hide();
    $$("DIV#ExampTabs-mainDiv DIV#ExampTabs-TABS DIV.results-hlevelError")[0].hide();
	...
	if (queryDef.length == 0) {
	    $$("DIV#ExampTabs-mainDiv DIV#ExampTabs-TABS DIV.results-invalidQuery")[0].show();
	    return;
	}
	...
	for (var pnr = 0; pnr < panels.length; pnr++) {
	    ...
	    var panelDateFrom = i2b2.ExampTabs.extractDate(i2b2.h.getXNodeVal(panels[pnr],'panel_date_from'));
	    var panelDateTo   = i2b2.ExampTabs.extractDate(i2b2.h.getXNodeVal(panels[pnr],'panel_date_to'));
		...
		if (hlevel != 7 && hlevel != 8) {
		    $$("DIV#ExampTabs-mainDiv DIV#ExampTabs-TABS DIV.results-hlevelError")[0].show();
		    return;
		}
		itemsString 
		    += (itemsString != '' ? ', ' : '')
    ...
    i2b2.CRC.ajax.getRequestXml_fromQueryMasterId("Plugin:ExampTabs", msg_vals, scopedCallback);
    
    i2b2.ExampTabs.model.dirtyResultsData = false;		
}

i2b2.ExampTabs.extractDate = function(string) {
    if (string) {
